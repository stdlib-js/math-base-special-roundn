...
var roundn = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof roundn, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function supports rounding a numeric value to a desired number of decimals', opts, function test( t ) {
	t.strictEqual( roundn( PI, -2 ), 3.14, 'equals 3.14' );
	t.strictEqual( roundn( -PI, -2 ), -3.14, 'equals -3.14' );
	t.strictEqual( roundn( 9.99999, -2 ), 10.0, 'equals 10' );
	t.strictEqual( roundn( -9.99999, -2 ), -10.0, 'equals -10' );
	t.strictEqual( roundn( 0.0, 2 ), 0.0, 'equals 0' );
	t.strictEqual( roundn( 12368.0, -3 ), 12368.0, 'equals 12368' );
	t.strictEqual( roundn( -12368.0, -3 ), -12368.0, 'equals -12368' );
	t.end();
});
...
tape( 'the function supports rounding a numeric value to a desired number of digits', opts, function test( t ) {
	t.strictEqual( roundn( PI, 3 ), 0.0, 'equals 0' );
	t.strictEqual( roundn( 12368.0, 3 ), 12000.0, 'equals 12000' );
	t.strictEqual( roundn( 12368.0, 1 ), 12370.0, 'equals 12370' );
	t.strictEqual( isNegativeZero( roundn( -PI, 3 ) ), true, 'equals -0' );
	t.strictEqual( roundn( -12368.0, 3 ), -12000.0, 'equals -12000' );
	t.strictEqual( roundn( -12368.0, 1 ), -12370.0, 'equals -12370' );
	t.end();
});
